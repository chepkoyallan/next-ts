...
_topics_ and each subscriber creates a _subscription_ and consumes _messages_
from it.
...
- **Message**: A message is a unit of data that flows in the system. It
  contains opaque data from the publisher along with its _attributes_.
...
- To receive messages via pull, a subscriber calls the `Pull` method on the
  ...
  `PullResponse` to `Acknowledge` the message, or modify the _ack deadline_
...
Furthermore, the system guarantees _at-least-once_ delivery of each message
until acknowledged.
